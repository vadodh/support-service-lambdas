...
import com.gu.cancellation.sf_cases.Handler.SfClient
...
class EndToEndHandlerEffectsTest extends FlatSpec with Matchers {
  ...
  //
  //  def getCaseSteps(sfClient: SfClient)(apiGatewayRequest: ApiGatewayRequest) =
  //    (for {
  //      pathParams <- apiGatewayRequest.pathParamsAsCaseClass[CasePathParams]()
  //      sfGet = SalesforceCase.GetById[JsValue](sfClient.wrapWith(JsonHttp.get))_
